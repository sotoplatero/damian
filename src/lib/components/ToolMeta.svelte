<script lang="ts">
	...
	/**
	 * Los metadatos de una herramienta: título, descripción y la tarjeta que se ve
	 * al compartir el enlace.
	 *
	 * Está aquí y no repetido en cada página porque son ocho etiquetas y es muy
	 * fácil olvidarse de una. Cada herramienta pasa su título y su descripción; la
	 * imagen la deduce de la URL, así que no hay que pasarla ni mantenerla:
	 * `/tool/newsletter` -> `/og/newsletter.png` (ver src/routes/og).
	 *
	...
	const slug = $derived(page.url.pathname.replace(/^\/tool\//, '').replace(/\/$/, ''));
