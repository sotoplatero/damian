...
import { sendCervantesEmail, subscribe } from '$lib/server/resend';

const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

/**
 * The delivery of `/recursos/cervantes`: an address in, the download link out by
 * email.
 *
...
export const POST: RequestHandler = async ({ request, url, getClientAddress }) => {
	...
	try {
		...
		console.error('[recursos/cervantes] subscribe failed:', error);
		return json({ error: 'server_error' }, { status: 500 });
	}

	try {
		await sendCervantesEmail(email, url.origin);
	} catch (error) {
		console.error('[recursos/cervantes] delivery failed:', error);
