...
**`pnpm check` reports 0 errors and 0 warnings, and that is the baseline now.** It was
1 known error for a long time (a `deserialize` result typed `{}` in `places-evaluator`);
that one is fixed, so anything `check` says from here on is yours. Keep this line honest:
a stale baseline is worse than none, because it hides the error you added.
...
A fresh git worktree needs nothing generated before `pnpm check` is trustworthy. It used
to: `src/lib/paraglide/` was written by a Vite plugin and gitignored, so a clean checkout
...
- **Internationalization**: none. Spanish only, copy written where it is used
...
| `archive` | Listed. Downloads **somebody else's** whole archive as a zip. The only tool whose deliverable is a file served in the response, the only one with no free step, and the only one limited to one per address. See below. |
...
302s to the page), so a 1333-post archive is 1333 requests and no serverless function can sit
through it. So the work is cut into batches and **the tab is what accumulates state**: `start`
reads the index once and returns it, `batch` returns a slice of bodies and says how many slugs
it consumed, and `src/routes/tool/archive/+page.svelte` keeps asking until the archive runs
out — then builds the zip itself. There is no store in this project and no cron to drive one,
and this shape needs neither.

Two earlier shapes are recorded here because both looked right:

- **One request that did everything.** It fit the clock and not the archive: the bodies had to
  be capped, and a complete archive was impossible by construction.
- **A free step that showed the index first.** There was nothing to show — the counts of
  somebody else's publication are a receipt, not a sample — so it cost a second request and
  protected nothing. The address is asked for in the first form because that is where the
  price is.
...
- **`batch` is signed, not sessioned.** `signPass`/`verifyPass` in `tokens.ts` bind a pass to
  ...
  carries its own claim is the only thing that works across instances.
- **One download per address** (`archiveExport`, a month-long window), checked with
  `remaining()` and only counted with `overLimit()` **after the index has been read**:
  `overLimit` counts every attempt, which is right for a limit of fifteen and cruel for a
  limit of one — a mistyped address would spend somebody's only go.
- **A restack is another person's post** and a podcast body is a player, so `bodyCandidates`
  drops both. The index keeps them.

**`BODY_CAP` is 150 and it is a PRODUCT decision — do not present it as a technical one.**
Two earlier caps (150, then 50) were read off a Substack that the previous measurement had
just hammered: **a drained bucket looks exactly like a small bucket**, and the second reading
shaped the whole tool around a wall that isn't there. From a rested bucket, **200 unique post
pages in a row draw no 429**, and one that does clears in about 31 s. The download stops at 150
and ASKS — «descargar estos 150» or «seguir con los N que faltan» — because MEASURED against
real publications, 150 posts is about the last year: honest-broker.com's newest 150 cover Aug
2025 → Aug 2026 and its actual last year is **147**. A count beats a date window twice over:
bounded cost whatever the publication's rhythm, and immune to the 447 Honest Broker posts dated
in the year 2000. What the date rule was after is information, so `monthsCovered` puts the
covered months on screen and in the README.

**Measured end to end, 12 August 2026, in the browser:** Kloshletter's 167 posts complete in
...
('deflate-raw'), a browser global that Node has had since 18, so **one implementation serves
the tab and the tests**; entries that deflate bigger than they started are stored instead.
Verified by extracting with `Expand-Archive` and `unzip`; its test reads the bytes back the way
an extractor does and inflates them with `node:zlib`, a second implementation checking the
first.

`src/lib/tools/archive/html.ts` converts `body_html` to markdown, and it is NOT
`newsletter.ts`'s `toPlainText`: that one flattens a body to count things in it, and nobody
reads the output. This one is the product. Substack's own furniture (subscribe box, share
button, paywall panel) is removed **by walking nested `<div>`s**, because the first `</div>`
is never the right one. Two traps recorded in its tests: matching the outer list first merged
`a` and `b` into `ab`, and the global ` ?\n ?` cleanup ate the indentation of nested items.
...
else's terminal. It installs with `npx skills add https://damiansoto.me/skill`.

**Three things make that one line work, all of them found by failing, all of them generated by
`skill/build.mjs`:**

1. **A discovery index at `/skill/.well-known/agent-skills/index.json` AND one at the root.**
   The `skills` CLI (vercel-labs/skills) discovers through `.well-known` before it will even
   consider downloading the URL, and **with a URL that carries a path it does NOT fall back to
   the root index** — «Not falling back to the root skills index because that would install
   every skill the host publishes». The root one serves bare `npx skills add
   https://damiansoto.me`. This is also why `/skill` can stay an HTML page for people: the CLI
   never asks for it.
2. **A `digest`, the zip's sha256, which the CLI verifies.** Generated, never hand-written: a
   new zip with the old digest breaks the install for everyone and the error names nothing.
3. **The zip is FLAT — `SKILL.md` at its root.** After the digest check the CLI does
   `files.get("SKILL.md")` on the extracted archive and returns null if it isn't there. With
   everything nested under `damiansoto/` the install failed with a «No matching skills» that
   says nothing about any of this. The cost is that unzipping by hand needs a destination
   folder: `unzip -d ~/.claude/skills/damiansoto`.

**`claude skill add` does not exist** — Claude Code 2.1.268 has `claude plugin`, not
`claude skill`. Installing from a URL is `npx skills add`, or the zip by hand.
...
### There is no i18n, and that was a removal

**Paraglide JS (inlang) was ripped out in August 2026. Don't put it back without a second
language to show for it.** It had been wired through six places for one page's worth of
strings: a Vite plugin generating `src/lib/paraglide/`, `messages/{en,es}.json`,
`project.inlang/`, a `handle` hook whose only job was to fill `%paraglide.lang%` in
`app.html`, a `reroute` hook stripping locale prefixes, and one `import * as m` in
`places-evaluator`. Everything else on the site already carried its copy where it is used.

What the removal bought, beyond one fewer dependency:

- **`pnpm check` went from 1 known error to zero**, and a fresh worktree no longer needs a
  build before `check` can be trusted — the generated folder was gitignored, so a clean
  checkout used to report errors that weren't real.
- **`src/hooks.ts` and `src/hooks.server.ts` are gone entirely.** Neither did anything else.
  If a `handle` hook is ever needed again, it starts from an empty file.
- `app.html` carries `lang="es"` literally. The site is Spanish; the placeholder was the only
  reason that value was ever dynamic. **Nothing in `app.html` explains that, on purpose:** a
  comment in there is not documentation, it is a payload — it shipped in the HTML of every
  page on every request until somebody grepped for it.
- **The dev server stopped reloading itself.** The plugin rewrote its own cache under
  `project.inlang/cache/` seconds after boot, Vite watched those files, and every rewrite
  forced a page reload — which killed anything long-running in the tab, `/tool/archive`'s
  download loop included. That was diagnosed while building it and it is the least obvious
  thing on this list.

`places-evaluator`'s eight strings now live in a `const t` at the top of its page, like every
other tool. Its remaining oddity is DaisyUI, not i18n.

### SvelteKit Configuration

- `adapter-auto` (auto-detects the platform)
- Preprocessors: vitePreprocess and mdsvex
- **No hooks.** `src/hooks.ts` and `src/hooks.server.ts` existed only for Paraglide and were
  deleted with it.
- `$lib` maps to `src/lib/`
