...
`pnpm check` currently reports **3 known errors**, none of them from the current work.
A clean run means "no *new* errors", not zero:

- `src/routes/tool/places-evaluator` — 1, the oldest.
- `src/lib/courses/report.ts` and `src/routes/course/[slug]/+page.svelte` — 2, both
  `Cannot find module '$lib/content'`. They arrived with the course feature (`99c9f01`):
  the markdown `parseCopy` helper the course imports was removed in August 2026 with the
  rest of the `src/lib/content` system, and the course still imports it.

Keep this count honest. A stale baseline is worse than none — it hides the error you added.
